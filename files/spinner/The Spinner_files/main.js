...
function getMenuTop() {
  ...
  stack.pop(); // remove current file name (or empty string)
	...
	var links = [["index.htm","Home"], ["numbers/index.html","Numbers"], ["algebra/index.html","Algebra"], 
	["geometry/index.html","Geometry"], ["data/index.html","Data"],  
	["measure/index.html","Measure"], ["puzzles/index.html","Puzzles"], ["games/index.html","Games"], ["definitions/index.html","Dictionary"],
	["worksheets/index.php","Worksheets"]];
	
	var linkLen = links.length;
	if (window.innerWidth<700) linkLen -= 3;   /* less items on narrow screens */
	if (window.innerWidth<520) linkLen -= 2;   
  ...
  for (var i=0; i<linkLen; i++) {
	...
	if (linkLen<links.length) {
		s += '<li><a href="#">More&#x25BC;</a>';
		s += '<ul style="text-align:right;">';
	  for (var i=linkLen; i<links.length; i++) {
			s += '<li><a href="' + urlStt + links[i][0] + '">' + links[i][1] +'</a></li>';
	  }
		s += '</ul>';
		s += '</li>';
...
function getAdTop() {
	...
	s += '     style="display:inline-block;width:468px;height:60px"';
	s += '     data-ad-client="ca-pub-1389989178296449"';
	s += '     data-ad-slot="6750421832"></ins>';
	s += '<script>';
...
function getLinks(oldQ) {
  ...
  if (oldQ) {
	...
	s += gPlusOneCall();
		...
		if (window.innerWidth<=320) {   /* looks better without on narrow screens */
		} else {
			s += '<div class="g-plusone" data-size="standard" data-href="' + url + '"></div>';   
		}
	}
